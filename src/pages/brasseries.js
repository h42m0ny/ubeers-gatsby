...
class BreweriesPage extends React.Component {
  ...
  searchBreweriesFromInput(value){
    ...
    if(value !== undefined && value.length >= 4){
      this.setState({
        breweries:_.filter(this.props.data.allStrapiBreweries.nodes,(brewery) => brewery.name.indexOf(value) !== -1)
      })
    }else{
      this.setState({
      ...
      <Layout>
        ...
        <Section>
          <Container>
            <div>
                <p>
                    Illud tamen te esse admonitum volo, primum ut qualis es talem te esse omnes existiment 
                    ut, quantum a rerum turpitudine abes, tantum te a verborum libertate seiungas; 
            ...
            <div>
                ...
                <InputStyled type="text" onChange={(e) => this.searchBreweriesFromInput(e.target.value)} placeholder={this.props.data.allStrapiBreweries.nodes[0].name}/>
            </div>
